...
subprocess.call(
    ['tabgtigen', 'table={0}'.format(rateset), 'gtiset={0}'.format(gtifile),
     'expression={0}'.format(gtiexp)])
...
subprocess.call(
    ['evselect', 'table={0}:EVENTS'.format(cleanevt), 'withrateset=yes',
     'rateset={0}'.format(cleanrate), 'maketimecolumn=yes',
     'makeratecolumn=yes', 'timecolumn=TIME', 'timebinsize=100',
