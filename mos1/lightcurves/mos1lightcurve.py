...
for bin in bins:
    for i, energy in enumerate(zip(emins, emaxs)):
        ...
        srclc = 'mos1_lc_src_{0}_bin{1}.ds'.format(strin[i], bin)
        bkglc = 'mos1_lc_bkg_{0}_bin{1}.ds'.format(strin[i], bin)
        netlc = 'mos1_lc_net_{0}_bin{1}.ds'.format(strin[i], bin)
        srcimg = 'mos1_src_img_{0}_bin{1}.ds'.format(strin[i], bin)
        bkgimg = 'mos1_bkg_img_{0}_bin{1}.ds'.format(strin[i], bin)
        psimg = 'mos1_lc_net_{0}_bin{1}.ps'.format(strin[i], bin)
