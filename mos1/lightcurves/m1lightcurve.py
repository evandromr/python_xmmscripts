...
for bin in bins:
    for i, energy in enumerate(zip(emins, emaxs)):
        ...
        srclc = 'm1_lc_src_{0}_bin{1}.ds'.format(strin[i], bin)
        bkglc = 'm1_lc_bkg_{0}_bin{1}.ds'.format(strin[i], bin)
        netlc = 'm1_lc_net_{0}_bin{1}.ds'.format(strin[i], bin)
        srcimg = 'm1_src_img_{0}_bin{1}.ds'.format(strin[i], bin)
        bkgimg = 'm1_bkg_img_{0}_bin{1}.ds'.format(strin[i], bin)
        psimg = 'm1_lc_net_{0}_bin{1}.ps'.format(strin[i], bin)

        # selection exprssion
        srcexp = 'expression=#XMMEA_EM && (PI IN [{0}:{1}]) && PATTERN <={3} \
&& FLAG==0 && ((X,Y) IN {2})'.format(energy[0], energy[1], srcregion, pattern)
        bkgexp = 'expression=#XMMEA_EM && (PI IN [{0}:{1}]) && PATTERN <={3} \
